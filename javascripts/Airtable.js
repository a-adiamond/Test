...
const Airtable = {
    ...
    Table: class {
        /**
         * Initialization options that can be passed to the `Airtable.Table()` constuctor.
         * @typedef {object} AirtableTableOptions
         ...
         * @param {AirtableTableOptions} options - An `AirtableTableOptions` object or just the table name.`
         ...
         * have zero empty fields, Airtable will not be able to accurately identify all fields.
         */
                ...
                pull: callback => {
                    if (_props.options.name && _props.options.baseID && _props.options.apiKey) {
                        ...
                        xhr.GET(_ => {
                            ...
                            console.log("1",xhr.response.records)
                            _props.offset = xhr.response.offset
                            if (_props.offset) {
                                const xhr2 = new XHR()
                                    xhr2.GET(_ => {
                                    _props.records = Object.assign(_props.records,xhr2.response.records) 
                                    console.log("2",xhr2.response.records)
                                    _props.offset = xhr2.response.offset
                                    console.log("2.5?",Object.assign(xhr.response.records,xhr2.response.records))
                                    if (_props.offset) {
                                        const xhr3 = new XHR()
                                         xhr3.GET(_ => {
                                           _props.records =  Object.assign(_props.records,xhr3.response.records)
     ...
     * @name Table#options
     * @function @memberof Table
     * @returns {AirtableTableOptions} Options originally passed to the constructor.
     */
